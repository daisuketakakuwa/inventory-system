...
func main() {
	...
	{
		logger = log.New(os.Stderr, "[inventorysystem] ", log.Ltime)
	}
	...
	{
		inventorySvc = inventorysystem.NewInventory(logger)
	}

	// Load .env file
	database.LoadEnvFile()
	// Setup Database
	database.SetupDb()
	...
	case "localhost":
		{
			...
			handleHTTPServer(ctx, u, inventoryEndpoints, &wg, errc, logger, *dbgF)
		}

		{
			addr := "grpc://localhost:8088"
			u, err := url.Parse(addr)
			if err != nil {
				fmt.Fprintf(os.Stderr, "invalid URL %#v: %s\n", addr, err)
				os.Exit(1)
